...
export async function fetchInterestedProperties(): Promise<string[]> {
  ...
  // Convert full property objects to _id array
  const propertyIds: string[] = (data?.data ?? []).map((item: any) => item._id)
...
export async function deleteInterestedProperty(payload: { property_id: string }): Promise<{ success: boolean }> {
  const token = getAuthToken();
  if (!token) {
    throw new Error('Authentication token not found');
  }

  const res = await fetch(`${API_BASE_URL}/${API_ENDPOINT.DELETE_INTERESTED_PROPERTY}`, {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': token,
  ...
  if (!res.ok) {
    const data = await res.json();
    throw new Error(data?.message || 'Failed to remove interested property');
  }
