...
export default function ItemDetails() {
  ...
  const handleInterestedButtonClick = async () => {
    ...
    const res = await fetch('/zoho/crm/v7/Deals', {
      method: 'POST',
      headers: {
        Authorization: `Zoho-oauthtoken ${accessToken}`,
        'Content-Type': 'application/json',
    ...
    <div className='max-w-[980px] mx-auto'>
      <div className='border p-4 bg-slate-100 rounded-md'>
        <h1 className='text-2xl font-bold flex items-center justify-between'>
          <p>
            {data?.Bank_Name} Auctions for {data?.Property_Type} in {data?.Area}
            , {data?.City}
          </p>
          <button onClick={handleInterestedButtonClick} disabled={isInterested}>
            <Heart
      ...
      <div className='p-4 bg-slate-100 mt-5 rounded-md flex justify-between flex-wrap gap-5'>
        ...
        <div className='*:py-2'>
          ...
          <div>
            <span className='font-bold'>Branch Name:</span> {data?.Branch_Name}
          </div>
        </div>
        <div className='*:py-2'>
          <p>
