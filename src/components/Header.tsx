...
function useActiveSection() {
  ...
  return { active: manualActive || active, setManualActive }
}
...
function NavLinks() {
  ...
  const handleClick = (
    ...
    if (id) {
      e.preventDefault()
      setManualActive(id) // highlight immediately

      const el = document.getElementById(id)
      if (el) {
        el.scrollIntoView({ behavior: 'smooth', block: 'start' })
        // remove manual highlight after scroll ends
        setTimeout(() => setManualActive(null), 800)
      }
    }
  }
    ...
    <ul className='hidden tablet:flex items-center gap-5 lg:gap-8'>
      {NAV_LINKS.map((item) => {
        ...
        return (
          ...
          >
            <li
              className={`px-0 cursor-pointer font-outfit text-center font-semibold text-2xl transition-colors py-2 ${
...
export default function Header() {
  ...
  const { setManualActive } = useActiveSection()
          ...
          <div className='hidden min-[1250px]:flex items-center gap-6 mt-4 min-[1250px]:mt-0 w-full min-[1250px]:w-auto justify-end'>
            {isLoggedIn ? (
              ...
              <button
                onClick={() => { setActiveTab('login'); setAuthOpen(true) }}
              >
                <div className='inline-flex items-center justify-center gap-2 h-10 px-4 py-2 w-full tablet:w-auto bg-[#E34732] text-white hover:bg-auction-gold hover:text-auction-navy rounded-sm cursor-pointer'>
